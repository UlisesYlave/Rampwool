...
contract NFTMarketplace is IERC721Receiver, ReentrancyGuard, Pausable, Ownable {
    ...
    constructor(uint256 _marketplaceFee, address _feeRecipient) Ownable() {
    ...
    function _executeSale(
        ...
        try IERC2981(nftContract).royaltyInfo(tokenId, price) returns (
            ...
            if (
                receiver != address(0) && amount > 0 && amount <= sellerAmount
            ) {
        ...
        _salesHistory[nftContract][tokenId].push(
            Sale({
                seller: seller,
                buyer: buyer,
                price: price,
